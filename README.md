...
Currently, you can use four different sources for literature data: Data Files (BibTeX and CSL-JSON), Zotero/Juris-M (using Better BibTeX plugin), Zotero/Juris-M (using debug-bridge plugin), and Zotero/Juris-M (using Web API). The following table provides a comparison between the four data sources:

| Data Source | BibTeX and CSL-JSON | Zotero/Juris-M with Better BibTeX plugin | Zotero/Juris-M with debug-bridge plugin | Zotero/Juris-M with Web API |
|:------------|:-------------------:|:-------------------------------------:|:--------------------------------------:|:-----------------------------:|
| Literature Identifier | <b>citekey</b><br>Allows multiple files<br>Ensure uniqueness on your own | <b>citekey</b><br>Generated by BBT | <b>citekey / itemKey</b><br>citekey generated by BBT<br>itemKey generated by Zotero | <b>itemKey</b><br>Generated by Zotero |
| Search Panel | Citation plugin panel | Zotero panel | Citation plugin panel / Zotero panel (optional) | Citation plugin panel |
| Initialization Speed | Slow | Fast | Fast | Fast |
| Search/Insert Speed | Fast | Slow | Fast (Citation plugin panel) | Fast |
...
| Export to LaTeX with Citations | × | × | ✓ (requires better-bibtex) | ✓ (requires better-bibtex) |
...
4. Choose [Database](#glossary) Type: Select the type of database you want to use. If using data files, please refer to [Using Data Files as Literature Data Source](#using-data-files-as-literature-data-source). If using Zotero/Juris-M (using Better BibTeX plugin), refer to [Using Zotero/Juris-M (using Better BibTeX plugin) as Literature Data Source](#using-zoterobetter-bibtex-as-literature-data-source). If using Zotero/Juris-M (using debug-bridge plugin), refer to [Using Zotero/Juris-M (using debug-bridge plugin) as Literature Data Source](#using-zoterojuris-m-debug-bridge-as-literature-data-source). If using Zotero/Juris-M (using Web API), refer to [Using Zotero/Juris-M (using Web API) as Literature Data Source](#if-you-use-zoterojuris-m-web-api-as-literature-data-source).
...
### If You Use Zotero/Juris-M (Web API) as Literature Data Source

- **Make sure you are using Zotero 7 / Juris-M, enable `Settings -> Advanced -> Allow other applications on this computer to communicate with Zotero`, and ensure the local Web API is reachable. No Better BibTeX or debug-bridge plugin is required.**
- In the plugin's settings panel, select `Zotero (Web API)` or `Juris-M (Web API)` as the database type.
- Web API mode **always uses itemKey as the index**. This mode is intended for users who want a simpler setup and do not want to install additional Zotero plugins.
- Ensure that your Zotero or Juris-M is open before using the plugin.
- Current scope:
  - Reading items, attachments, and notes is supported.
  - Basic attachment import works in the SiYuan desktop app.
  - PDF annotation support is currently limited; for the most complete annotation workflow, `debug-bridge` is still recommended.
...
For `Zotero/Juris-M (Web API)` mode, the usable scope is:

- **Best support**: SiYuan desktop app with Zotero running on the same machine
- **Possible**: web/Docker deployment when the Zotero local API port is reachable from the browser or through the local network, and CORS is handled correctly
- **Not guaranteed**: mobile/browser-only environments that cannot directly access the Zotero local API port

In addition, some attachment import behavior in Web API mode relies on the desktop Node/Electron environment, so the desktop app currently provides the most complete experience.
...
Starting from version 0.4.0, the Citation plugin can add literature documents to a SiYuan database and update database attributes while citing.

1. Enter the target database's **Database Block ID**.
2. Wait for the plugin to load its columns.
3. Choose a fill mode for each column. Select **Do not fill** for columns that the plugin should not maintain.

Built-in modes include:

- Select/multi-select: First author, Last author, and Journal;
- Date: Publication date;
- URL: URL and Zotero URL;
- Every column type: Custom.

Built-in modes safely handle empty values and JSON special characters. Legacy database attribute templates are migrated to per-column **Custom** settings only after the database columns load successfully. Columns absent from the legacy template remain disabled. If loading or migration fails, the original settings are preserved instead of being replaced with an empty configuration.

![](./assets/database.png)

### Custom Database Attributes

When **Custom** is selected, enter only that column's `value` object. Do not include `keyID` or the outer list. For example, a text column can use:
...
A multi-select tag column can use:
...
Use `JSON.stringify(...)` for string output. Do not place `{{ ... }}` directly inside JSON quotes, because quotes, backslashes, or line breaks in literature metadata would produce invalid JSON.
